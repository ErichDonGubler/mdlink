...
pub(crate) fn try_write_markdown_url<'a>(
    ...
    let orgs = config.map(|gitlab| &gitlab.orgs);
    ...
    if let Some((org, repo)) = path_segments.next_tuple() {
        ...
        if let Some(("-", ty @ ("issues" | "merge_requests"), issue_num)) =
            path_segments.clone().next_tuple()
        {
            let ref_prefix = match ty {
                "issues" => "#",
                "merge_requests" => "!",
                _ => unreachable!(),
            };
            write!(f, "[{repo_prefix}{ref_prefix}{issue_num}]({url})")?;
            return Ok(Some(FancyMarkdownMatched::Yes));
        }
            ...
            match path_segments.next_tuple() {
                ...
                Some(("-", "releases", tag)) => {
                    match (path_segments.next(), path_segments.next()) {
                        (Some(""), None) | (None, ..) => {
                            static COMPONENT_VERSION_RE: OnceLock<regex::Regex> = OnceLock::new();
